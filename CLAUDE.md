...
There is reportedly a formal **SDD (Software Design Document)** that is the actual source of truth for scope — **its location is not yet known to Claude**. A flowchart image was shared once (2026-08-14) summarizing planned modules/roles/integrations, but the user clarified that was just illustrative conversation, not the authoritative doc. If you need to check something against the SDD and don't have it, ask the user where it lives before assuming a flowchart or verbal description is complete/current.
...
## 10. Where to look for more history
...
- The auto-memory system (Claude-side, not in this repo) has two files worth knowing exist even if you can't read them directly: one on commit-granularity feedback, one on the GPS-attendance feature's evolving architecture and decisions — this document has folded their content in, but they may have been updated further since this file was written.
