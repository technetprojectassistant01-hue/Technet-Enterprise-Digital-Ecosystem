...
### 7a. GPS site attendance — current architecture (rewritten 2026-09-03)

**There is exactly one check-in surface**: the "My Attendance" widget on the dashboard Overview page (`client/src/dashboard/AttendanceWidget.tsx`), calling `POST /api/site-attendance/check-in`. No daily limit — a technician can check in/out repeatedly as they visit different sites in a day. Only employees with a linked `User` account see the widget at all.

**What the technician fills in**: a time (`<input type="time">`, prefilled from the clock), a location, and an optional transport cost in MUR. Check-out mirrors it — time out, an optional location, and the return leg's cost. Every submission also carries a GPS fix and the server's own timestamp.
...
- `UNCHECKABLE` is the **normal** outcome and is not suspicion. Free text like "Office" has nothing to compare against, and business names don't resolve either (§7b). It renders as no marker at all, and is excluded from the summary count.
- The 10km radius is deliberately generous because a place name resolves to an **area centroid**, not a building: from a real check-in, "Réduit" measured 5.6km away and "Ébène" 6.4km. Mauritius is ~65km end to end, so this catches naming one part of the island while standing in another. It **cannot** tell whether somebody is at the right building, and must not be described as though it can.
...
**A second, work-order-specific check-in flow was briefly built (2026-08-13) then fully retired the same day** after the user caught that it was the wrong shape — the trust/tracking features were supposed to extend the *existing* daily check-in, not add a competing one on the Work Order Detail page. `WorkOrderDetailPage.tsx` is now purely read-only for attendance. **If asked to touch check-in behavior again: do not add check-in controls to the work order page.** Everything belongs on `AttendanceWidget.tsx`.

**Forgotten sessions — a manager can close one, nothing closes it automatically** (`1ff2eb8`). One real session ran from 27 August to 3 September and booked a ~163-hour visit, distorting that month's "hours on site"; and since Field Operations shows all open sessions, a forgotten check-in reads as "in the field" indefinitely. `POST /api/site-attendance/:id/close` (`OPS_MANAGE_ROLES`) now fixes one by hand, surfaced as a Close action on **Team Attendance** (not Field Operations). Two deliberate properties: it records **no coordinates** (nobody observed where the technician was, and inventing a position would put fabricated data into the record this system exists to be trusted on), and `checkOutAt` is **optional and expected to be corrected** — the picker defaults to now, but a manager who knows the technician actually left at 17:00 last Thursday says so, otherwise the close itself books the same inflated hours. `checkOutByManager` marks it so an administrative close never reads as a real one. **Still genuinely absent**: any automatic/scheduled close, so a session nobody notices stays open indefinitely.
